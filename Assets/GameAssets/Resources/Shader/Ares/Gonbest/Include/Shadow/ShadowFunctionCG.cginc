...
Author:gzg
Date:2019-08-20
Desc:Custom shadow processing, sampling through the program-generated shadow map, and then performing projection processing.
======================================================================*/
...
/*******************ShadowMap shadow processing****************************/
	...
	//Calculate the sampling UV value of the Shadowmap, and adjust the starting position of the sampling point to the lower left corner.
	...
	//Calculate the depth value, convert the depth value to the HDC[0,1] interval, and then sort the depth by rule Near:0->Far:1
	...
	// Encode the depth: use direct value for depth textures, fallback to RGBA encoding if needed
	inline float4 EncodeDepth(float depth)
	{
	#if defined(_SHADOWMAP_NATIVE_DEPTH)
		// Store depth directly in .r (for native depth textures or single channel)
		return float4(depth, 0, 0, 1);
	#else
		// Fallback: encode to RGBA
		return EncodeFloatRGBA(depth);
	#endif
	}

	// Decoding Depth: use direct value for depth textures, fallback to RGBA decoding if needed
	...
	{
		//Precision - using a lower value for better device compatibility
		float TransitionScale = 4000;
		return saturate(1 - (SceneDepth - ShadowmapDepth) * TransitionScale);
	}

	//After texture sampling, comparison is performed to obtain the bool value 0,1
