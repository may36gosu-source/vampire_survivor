//模型死亡时的Shader
Shader "Ares/EntityState/Dead_RGB"
{
	Properties
	{
		_MainTex ( "Base (RGB)", 2D ) = "white" {}
		_AlphaTex("alpha (AAA)", 2D) = "white" {}
		_GrayTime ( "Gray time", float ) = 2.0
		_DissTime ( "Dissolve time", float ) = 2.0
		_Timeline ( "Timeline", Range( 0, 5.0 ) ) = 0.0	
		[HideInInspector]_Color ("Main Color", Color) = (1, 1, 1, 1)		
		[HideInInspector]_ColorMultiplier("Color Multipler",range(0,2)) = 1
	...
	SubShader
	{
        Tags {"Queue" = "AlphaTest+50" "RenderType" = "TransparentCutout" "GonbestBloomType"="BloomMask"}
        //LOD 100
        Lighting Off
        UsePass "Gonbest/Function/DissolveHelper/DISSOLVE&GRAY"
