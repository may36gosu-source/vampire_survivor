Shader "leeveel/GrassAni_Mask"
{
  Properties
  {
    ...
    _CutClip ("半透明裁剪阈值", Range(0, 1.01)) = 0.5
    _MinY ("MinY", float) = 1
    _Mask ("Mask", 2D) = "black" {}
  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: 
    {
      ...
      OUT_Data_Vert vert(appdata_t in_v)
      {
          ...
          u_xlat0.y = (_Time.y * _YLoopSpeed);
          u_xlat0.xy = sin(u_xlat0.xy);
          u_xlat0.xy = (u_xlat0.xy * float2(_OffsetPower, _OffsetPower));
          ...
          u_xlat4 = max(u_xlat1.y, _MinY);
          u_xlat4 = min(u_xlat4, 1000);
          u_xlat4 = (float(1) / u_xlat4);
          u_xlat0.xy = (float2(u_xlat4, u_xlat4) * u_xlat0.xy);
          u_xlat4 = tex2Dlod(_Mask, float4(float3(in_v.texcoord.xy, 0), 1)).x;
          u_xlat0.xy = ((u_xlat0.xy * float2(u_xlat4, u_xlat4)) + u_xlat1.xz);
          u_xlat1.xyz = (u_xlat1.yyy * conv_mxt4x4_1(unity_WorldToObject).xyz);
          u_xlat0.xzw = ((conv_mxt4x4_0(unity_WorldToObject).xyz * u_xlat0.xxx) + u_xlat1.xyz);
          u_xlat0.xyz = ((conv_mxt4x4_2(unity_WorldToObject).xyz * u_xlat0.yyy) + u_xlat0.xzw);
