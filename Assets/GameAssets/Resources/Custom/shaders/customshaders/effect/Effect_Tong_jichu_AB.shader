Shader "Effect/Tong_jichu_AB"
{
  Properties
  {
    ...
    [MaterialToggle] _World_Mask_View ("World_Mask_View", float) = 0
  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: FORWARD
    {
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          u_xlat8.xy = ((float2(_Tex_Mask_U_Z, _Tex_Mask_V_W) * u_xlat1_d.xy) + u_xlat8.xy);
          u_xlat0_d.xy = (u_xlat0_d.xy + u_xlat8.xy);
          u_xlat1_d.xy = (u_xlat8.xy + in_f.texcoord2.xy);
          u_xlat8.xy = (u_xlat8.xy + in_f.texcoord.xy);
          u_xlat0_d.xy = (u_xlat0_d.xy + (-u_xlat1_d.xy));
          u_xlat0_d.xy = ((float2(float2(_World_Mask_View, _World_Mask_View)) * u_xlat0_d.xy) + u_xlat1_d.xy);
          u_xlat0_d.xy = ((-u_xlat8.xy) + u_xlat0_d.xy);
          u_xlat0_d.xy = ((float2(float2(_World_Mask, _World_Mask)) * u_xlat0_d.xy) + u_xlat8.xy);
          ...
          u_xlat4.x = (u_xlat16_1.w * _Color.w);
          u_xlat0_d.x = (u_xlat0_d.x * u_xlat4.x);
          u_xlat0_d.x = (u_xlat0_d.x * in_f.color.w);
          u_xlat4.x = (in_f.texcoord1.x + (-1));
          u_xlat4.x = ((_Color_Alpha_X * u_xlat4.x) + 1);
          out_f.color.w = (u_xlat4.x * u_xlat0_d.x);
          u_xlat0_d.x = dot(u_xlat16_1.xyz, float3(0.300000012, 0.589999974, 0.109999999));
          u_xlat0_d.xyz = ((-u_xlat16_1.xyz) + u_xlat0_d.xxx);
          u_xlat0_d.xyz = ((float3(_Desaturate, _Desaturate, _Desaturate) * u_xlat0_d.xyz) + u_xlat16_1.xyz);
          u_xlat1_d.xyz = (u_xlat0_d.xyz * _Color.xyz);
          u_xlat1_d.xyz = (u_xlat1_d.xyz * in_f.color.xyz);
          u_xlat0_d.xyz = ((_Color.xyz * u_xlat0_d.xyz) + (-u_xlat1_d.xyz));
          out_f.color.xyz = ((float3(float3(_Vertex_Color, _Vertex_Color, _Vertex_Color)) * u_xlat0_d.xyz) + u_xlat1_d.xyz);
          return out_f;
      }
